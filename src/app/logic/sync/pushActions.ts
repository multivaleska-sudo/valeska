  ...
  try {
    ...
    if (payload.clientes?.length)
      await markRecordsAsSynced(
        sqlite,
        "clientes",
        payload.clientes.map((r: any) => r.id),
      );
    if (payload.vehiculos?.length)
      await markRecordsAsSynced(
        sqlite,
        "vehiculos",
        payload.vehiculos.map((r: any) => r.id),
      );
    if (payload.empresasGestoras?.length)
      await markRecordsAsSynced(
        sqlite,
        "empresas_gestoras",
        payload.empresasGestoras.map((r: any) => r.id),
      );
    if (payload.representantesLegales?.length)
      await markRecordsAsSynced(
        sqlite,
        "representantes_legales",
        payload.representantesLegales.map((r: any) => r.id),
      );
    if (payload.presentantes?.length)
      await markRecordsAsSynced(
        sqlite,
        "presentantes",
        payload.presentantes.map((r: any) => r.id),
      );
    if (payload.tramites?.length)
      await markRecordsAsSynced(
        sqlite,
        "tramites",
        payload.tramites.map((r: any) => r.id),
      );
    if (payload.tramiteDetalles?.length)
      await markRecordsAsSynced(
        sqlite,
        "tramite_detalles",
        payload.tramiteDetalles.map((r: any) => r.id),
      );
    if (payload.sucursales?.length)
      await markRecordsAsSynced(
        sqlite,
        "sucursales",
        payload.sucursales.map((r: any) => r.id),
      );
    if (payload.dispositivos?.length)
      await markRecordsAsSynced(
        sqlite,
        "dispositivos",
        payload.dispositivos.map((r: any) => r.id),
      );
    if (payload.usuarios?.length)
      await markRecordsAsSynced(
        sqlite,
        "usuarios",
        payload.usuarios.map((r: any) => r.id),
      );
    if (payload.messageTemplates?.length)
      await markRecordsAsSynced(
        sqlite,
        "message_templates",
        payload.messageTemplates.map((r: any) => r.id),
      );
