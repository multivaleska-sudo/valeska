...
export function useCatalogosLogic() {
  ...
  const loadCatalogos = useCallback(async () => {
    try {
      ...
      sileo.error({
        title: "Error al cargar los catálogos de la base de datos",
      });
    }
  }, []);
  ...
  const saveSituacion = (situacion: Situacion) => {
    const promise = async () => {
      ...
      if (situacion.id) {
        ...
        const newId = crypto.randomUUID();
        await sqlite.execute(
          "INSERT INTO catalogo_situaciones (id, nombre, color_hex, activo, created_at, updated_at, sync_status) VALUES ($1, $2, $3, $4, $5, $6, $7)",
          [
            newId,
            situacion.nombre,
            situacion.colorHex,
