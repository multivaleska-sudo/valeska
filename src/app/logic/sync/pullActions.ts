export async function processPullSync(sqlite: any, pullData: any) {
    ...
    // Almacenamos en local los conflictos que el servidor nos avisa que existen
    for (const conf of pullData.conflictos || []) {
        await sqlite.execute(
            ...
            [
                ...
                typeof conf.datosLocales === 'string' ? conf.datosLocales : JSON.stringify(conf.datosLocales),
                typeof conf.datosRemotos === 'string' ? conf.datosRemotos : JSON.stringify(conf.datosRemotos),
                conf.resuelto ? 1 : 0,
