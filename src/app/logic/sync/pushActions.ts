...
export async function buildPushPayload(sqlite: any) {
  ...
  const catTiposRaw: any[] = await sqlite.select(
    "SELECT * FROM catalogo_tipos_tramite WHERE sync_status != 'SYNCED'",
  );
  const catSitRaw: any[] = await sqlite.select(
    "SELECT * FROM catalogo_situaciones WHERE sync_status != 'SYNCED'",
  );
  const clientesRaw: any[] = await sqlite.select(
    "SELECT * FROM clientes WHERE sync_status != 'SYNCED'",
  );
  ...
  const tramDetallesRaw: any[] = await sqlite.select(
    "SELECT * FROM tramite_detalles WHERE sync_status != 'SYNCED'",
  );
