...
export function useDashboardLogic() {
    const [stats, setStats] = useState<DashboardStats>({ activos: 0, clientes: 0, pendientes: 0, completadosMes: 0 });
    const [recentTramites, setRecentTramites] = useState<RecentTramite[]>([]);
    const [syncStats, setSyncStats] = useState<SyncStats>({ pendientes: 0, conflictos: 0, lastSync: 'Hace un momento' });
    ...
    const loadDashboardData = useCallback(async () => {
      ...
      `);
            ...
            const date = new Date();
            const firstDayOfMonthMs = new Date(date.getFullYear(), date.getMonth(), 1).getTime();
      ...
      `);
            ...
            const mappedRecientes: RecentTramite[] = recientesRes.map(r => {
                ...
                if (sitName.includes('entregado') || sitName.includes('finalizado')) {
                    status = 'success';
                } else if (sitName.includes('conflicto') || sitName.includes('observado')) {
                    status = 'conflict';
                }
    ...
    useEffect(() => {
        ...
        return () => window.removeEventListener("valeska_reload_tramites", handleReload);
    }, [loadDashboardData]);
