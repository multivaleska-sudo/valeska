...
pub fn parse_full_ubl_xml(path: &str) -> Result<FullInvoiceData, String> {
    ...
    if let Some(cap) = re_supplier.captures(&content) {
        ...
        em_ruc = re_id.captures(block).and_then(|c| c.get(1)).map(|m| m.as_str()).unwrap_or("---").to_string();
    ...
    if let Some(cap) = re_customer.captures(&content) {
        let block = cap.get(1).unwrap().as_str();
        rec_ruc = re_id.captures(block).and_then(|c| c.get(1)).map(|m| m.as_str()).unwrap_or("---").to_string();
    ...
    for (idx, cap) in re_line.captures_iter(&content).enumerate() {
        ...
        let (unidad, cantidad) = match re_qty.captures(block) {
            ...
            None => {
                let cant = re_qty_alt.captures(block).and_then(|c| c.get(1)).map(|m| m.as_str()).unwrap_or("1").to_string();
                ("NIU".to_string(), cant)
            }
        };
        ...
        let codigo = re_item_id.captures(block).and_then(|c| c.get(1)).map(|m| m.as_str()).unwrap_or("-").to_string();
        let precio = re_line_total.captures(block).and_then(|c| c.get(1)).map(|m| m.as_str()).unwrap_or("0.00").to_string();
