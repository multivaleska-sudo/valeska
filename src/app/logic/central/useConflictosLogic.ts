...
export function useConflictosLogic() {
    ...
    // 1. OBTENER CONTEO (Para CentralPage)
    ...
    const resolveConflicto = async (
        ...
        const promise = async () => {
            ...
            const keysToUpdate = Object.keys(resolvedData).filter(
                k => !['id', 'created_at', 'updated_at', 'deleted_at', 'sync_status'].includes(k)
            );

            if (keysToUpdate.length > 0) {
                const setClauses = keysToUpdate.map((key, index) => `${key} = $${index + 1}`).join(", ");
